...
for _ in {1..100}; do
  if grep -q 'State synchronized revision=4 temp=24 steps=9632 notifications=3' "${WATCH_LOG}"; then
    break
  fi
...
grep -q 'State synchronized revision=1 temp=24 steps=9000 notifications=3' "${WATCH_LOG}"
